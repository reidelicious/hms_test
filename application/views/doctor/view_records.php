      ...
      <span class="text-warning" style="font-family:verdana; font-size:14px;">SHOW: 
          <select style="margin-top:10px;" onChange="location = this.options[this.selectedIndex].value">
              <?php if(!$this->input->get('show')) { ?>
                  <option selected value="<?php echo base_url(); ?>view_records">History</option>
                  <option value="<?php echo base_url(); ?>view_records?show=1">Upcoming</option>
                  <option value="<?php echo base_url(); ?>view_records?show=2">Rejected</option>
              <?php }if($this->input->get('show') == '1'){ ?>
                  <option selected value="<?php echo base_url(); ?>view_records">History</option>
                  <option <?php echo $_GET['show'] == '1' ? 'selected' : ''?> value="<?php echo base_url(); ?>view_records?show=1">Upcoming</option>
                  <option <?php echo $_GET['show'] == '2' ? 'selected' : ''?> value="<?php echo base_url(); ?>view_records?show=2">Rejected</option>
...
$( document ).ready(function() {	
  if($('#filter').val() == '2'){
    ...
    oTable = $('#dataTables-1').dataTable({
      "bProcessing": true,
      "bServerSide": true,
      "sAjaxSource": '<?php echo base_url('doctor/datatable_upcomingAppointments'); ?>',
    ...
    oTable = $('#dataTables-1').dataTable({
      "bProcessing": true,
      "bServerSide": true,
      "sAjaxSource": '<?php echo base_url('doctor/datatable_historyOfAppointments'); ?>',
